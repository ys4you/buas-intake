...
{
	//This function is coppied from function "loadFile" on https://github.com/kcat/openal-soft/blob/master/examples/alplay.c
	ALenum err, format;
	ALuint buffer;
	SNDFILE* sndfile;
	SF_INFO sfinfo;
...
void SoundBuffer::clearBuffers()
{
	// Delete all sound effect buffers
	if (!soundEffectBuffers.empty()) {
		alDeleteBuffers(static_cast<ALsizei>(soundEffectBuffers.size()), soundEffectBuffers.data());
		soundEffectBuffers.clear();  // Clear the vector
	}
}
